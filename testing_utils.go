...
// MockLineNumberInPullRequestFile provides a mock implementation for testing pull request integration
// This simulates finding line numbers in diffs without requiring external dependencies
func (th *TestHelper) MockLineNumberInPullRequestFile(owner, repo string, pullNumber int, path, content string) (int, error) {
	th.t.Helper()

	// For testing purposes, return a mock line number based on content
	// This simulates what would happen in real PR diff parsing
